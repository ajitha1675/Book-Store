...
const Login = () => {
  ...
  const handleGoogleSignIn = async() =>{
    try {
        ...
        alert("Login successful with google account");
        navigate("/");
    } catch (error) {
      ...
      <div className="w-full max-w-sm mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
           ...
           <div>
            ...
            <button className="bg-blue-500 hover:bg-blue-800 text-white font-bold py-2 px-8 rounded focus:outline-none ">Login</button>
           </div>
        </form>
        <p className="align-baseline font-medium mt-4 text-sm">Have not account? Please <Link to="/register" className="text-blue-500 hover:text-blue-800">Register</Link></p>
